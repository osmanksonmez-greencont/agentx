...
from agentx.agent.subagent import SubagentManager
from agentx.agent.tools.cron import CronTool
from agentx.agent.tools.filesystem import EditFileTool, ListDirTool, ReadFileTool, WriteFileTool
from agentx.agent.tools.message import MessageTool
from agentx.agent.tools.registry import ToolRegistry
from agentx.agent.tools.shell import ExecTool
from agentx.agent.tools.spawn import SpawnTool
from agentx.agent.tools.web import WebFetchTool, WebSearchTool
...
from agentx.session.manager import Session, SessionManager

if TYPE_CHECKING:
    from agentx.config.schema import ChannelsConfig, ExecToolConfig, SelfEditConfig
...
class AgentLoop:
    ...
    def __init__(
        ...
        from agentx.config.schema import ExecToolConfig
        self.bus = bus
    ...
    def _register_default_tools(self) -> None:
        ...
        if self.team_orchestrator:
            from agentx.agent.tools.team import TeamSubmitGoalTool
            self.tools.register(
                TeamSubmitGoalTool(
        ...
        if self.self_edit_config and self.self_edit_config.enabled:
            from agentx.agent.tools.self_edit import SelfEditGuardTool
            self.tools.register(
                SelfEditGuardTool(
    ...
    async def _connect_mcp(self) -> None:
        ...
        from agentx.agent.tools.mcp import connect_mcp_servers
        try:
