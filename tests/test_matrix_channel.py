...
import agentx.channels.matrix as matrix_module
from agentx.bus.events import OutboundMessage
from agentx.bus.queue import MessageBus
from agentx.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from agentx.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("agentx.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "agentx.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("agentx.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "agentx.channels.matrix.asyncio.create_task", _fake_create_task
    )
